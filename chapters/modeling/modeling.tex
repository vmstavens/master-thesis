...
\begin{equation} 
	...
	\label{eq:hf}
\end{equation}
where \mvar{f_c} is the magnitude of the contact force, \mvar{f_t} is the magnitude of the tangential force, \mvar{f_x}, \mvar{f_y} and \mvar{f_z} are the magnitudes of the \mvar{x}, \mvar{y} and \mvar{z} components of the contact force ($\vec{f}_c$) and \mvar{\mu} is the friction coefficient~\cite[Chapter 37]{handbook-of-robotics}. The angle of the cone \mvar{\beta} can be computed based on \mvar{\mu} as
%
\begin{equation} 
	\beta = \tan\inv (\mu).
